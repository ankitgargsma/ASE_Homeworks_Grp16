...
def pretty_print_rule(rule):
    merged_ranges = []
    ranges = rule.parts.values()
    for range_list in ranges:
        for range_item in range_list:
            merged = False
            for merged_range in merged_ranges:
                if merged_range['lo'] == range_item.x.lo and merged_range['hi'] == range_item.x.hi - 1:
                    merged_range['hi'] = range_item.x.hi
                    merged = True
                    break
                elif merged_range['lo'] == range_item.x.lo - 1 and merged_range['hi'] == range_item.x.hi:
                    merged_range['lo'] = range_item.x.lo
                    merged = True
                    break
                elif merged_range['lo'] == range_item.x.lo - 1 and merged_range['hi'] == range_item.x.hi + 1:
                    merged_range['lo'] = range_item.x.lo
                    merged_range['hi'] = range_item.x.hi
                    merged = True
                    break
            if not merged:
                merged_ranges.append({'lo': range_item.x.lo, 'hi': range_item.x.hi})
    
    pretty_ranges = []
    for merged_range in merged_ranges:
        pretty_range = f"{merged_range['lo']} < x < {merged_range['hi']}"
        pretty_ranges.append(pretty_range)
    
    return ' and '.join(pretty_ranges)